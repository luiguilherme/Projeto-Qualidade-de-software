import { FlexCardMixin } from "vlocity_cmt/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_cmt/flexCardUtility";
          ...
          export default class cfCpqCatalogSearch extends FlexCardMixin(LightningElement){
              @api debug;
              @api recordId;
              @api objectApiName;
              ...
              @track Label={CPQCategory:"Category",
      CPQSearch:"Search"
      };
              pubsubEvent = [];
              customEvent = [];
        ...
        this.pubsubEvent[0] = {
          basetypeaheadinputchange: this.handleEventAction.bind(this, data.events[0],0),
baseinputvaluechange: this.handleEventAction.bind(this, data.events[2],2)
        };

        pubsub.register(`multiselectcataloglist`,this.pubsubEvent[0]);

        this.pubsubEvent[1] = {
          cataloglist: this.handleEventAction.bind(this, data.events[1],1),
selectedcatalogsevent: this.handleEventAction.bind(this, data.events[3],3)
        };
        ...
        pubsub.register(`cpq_${this.recordId}`,this.pubsubEvent[1]);
              ...
              unregisterEvents(){
                pubsub.unregister(`multiselectcataloglist`,this.pubsubEvent[0]);
