import { FlexCardMixin } from "vlocity_cmt/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "vlocity_cmt/flexCardUtility";
      ...
      CPQViewBy:"View By"
        ...
        this.pubsubEvent[0] = {
          cpq_update_cart_detailview: this.handleEventAction.bind(this, data.events[0],0)
        };

        pubsub.register(`cpq_ui_event_${this.recordId}`,this.pubsubEvent[0]);

        this.pubsubEvent[1] = {
          baseinputvaluechange: this.handleEventAction.bind(this, data.events[1],1)
        };

        pubsub.register(`cpqcartdetailoptions`,this.pubsubEvent[1]);

        this.pubsubEvent[2] = {
          cpq_cart_updated: this.handleEventAction.bind(this, data.events[2],2),
cpq_load_more_cart: this.handleEventAction.bind(this, data.events[3],3),
cpq_spinner: this.handleEventAction.bind(this, data.events[4],4)
        };
        ...
        pubsub.register(`cpq_${this.recordId}`,this.pubsubEvent[2]);
              ...
              unregisterEvents(){
                pubsub.unregister(`cpq_ui_event_${this.recordId}`,this.pubsubEvent[0]);
pubsub.unregister(`cpqcartdetailoptions`,this.pubsubEvent[1]);
