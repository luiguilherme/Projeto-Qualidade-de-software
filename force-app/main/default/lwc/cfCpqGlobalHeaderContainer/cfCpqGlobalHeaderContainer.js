...
cpq_delete_item: this.handleEventAction.bind(this, data.events[3],3)
        };
        ...
        pubsub.register(`cpqQuantityField_${this.recordId}`,this.pubsubEvent[1]);

        this.pubsubEvent[2] = {
          baseinputvaluechange: this.handleEventAction.bind(this, data.events[2],2)
        };

        pubsub.register(`cpqProductAttributes`,this.pubsubEvent[2]);

              }

              unregisterEvents(){
                pubsub.unregister(`cpq_${this.recordId}`,this.pubsubEvent[0]);
pubsub.unregister(`cpqQuantityField_${this.recordId}`,this.pubsubEvent[1]);
pubsub.unregister(`cpqProductAttributes`,this.pubsubEvent[2]);
