...
export default class GetLineRadioPortability extends OmniscriptBaseMixin( LightningElement) {
//your lwc code here for radio group as shown above
    ...
    {
        this.isLoaded = false;
        if(data.selectedLine2.length != 0) {
            this.options = data.selectedLine2.options;
            this.numeroResultados = this.options.length;
            var obj = data.selectedLine2.options;
            this.idPesquisa = data.selectedLine2.idPesquisa;
        }
        else{
            this.options              = data.phones;
            var obj                   = data.phones;
            this.idPesquisa           = data.idPesquisa;
            console.log('showResults' + this.options);
            console.log('showResults' + this.idPesquisa);
            console.log('showResults' + data.capacityDemandAmount);
            console.log('showResults' + this.isLoaded);
        }
        ...
        this.isLoaded = true;
        console.log('showResultsFinal' + this.isLoaded);
    }
    ...
    handleGetMSISDN(event)
    {
        ...
        this.omniRemoteCall(params, true).then(result => {
            ...
            this.omniUpdateDataJson(this.selectedline2);
    ...
    pushItemToView(event)
    {
        ...
        this.selectedline2 = { line : selectedOption };
        this.statusICCID = { statusICCID : 'Valid'};
    ...
    getCheckedValue(){
        if(this.omniJsonData !== undefined){
            ...
            if(jsonData.hasOwnProperty('StepPortability')){
                jsonData = this.omniJsonData.StepPortability;   
                if(jsonData.hasOwnProperty('selectedLine2')){
                    if(jsonData.selectedLine2 == null) {
                        return ""
                    }
                    else {
                        var jsonDataLine = jsonData.selectedLine2.line;
                    }
                    return jsonDataLine;
                }
            }
